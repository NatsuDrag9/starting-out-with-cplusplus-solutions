...
/**
 * @brief Sorts the list in ascending order by the numeric valueo of the item stored in the node
 * Sorting algorithm: selection sort
 *
 */
void SimpleLinkedList::sort()
{
    if (head == NULL)
    {
        return;
    ...
    while (nodePtr->next != NULL)
    {
        ListNode *minNodePtr = nodePtr;
        ListNode *nextNodePtr = nodePtr->next;
        while (nextNodePtr != NULL)
        {
            if (nextNodePtr->value < minNodePtr->value)
            {
        ...
        // Swap values of the nodes and not the nodes themselves
        double tempValue = nodePtr->value;
        nodePtr->value = minNodePtr->value;
        minNodePtr->value = tempValue;
...
SimpleLinkedList::~SimpleLinkedList()
{
    CopyLinkedList::destroy_list(head);
}
...
/**
 * @brief Destroys the list by deallocating all of its nodes using base class's destructor
 *
 * @param node - head node of the list to be deleted
 */
void CopyLinkedList::destroy_list(ListNode *node)
{
    while (node != NULL)
    {
        ListNode *temp = node;
