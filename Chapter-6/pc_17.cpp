...
{
    ofstream outputFile;
    outputFile.open("pc_17_in_patient.txt");
    if (outputFile.fail())
    {
        cout << "Error! Could not create output file. Exiting..." << endl;
        exit(0);
    }
    outputFile << "\nBase charges (in ₹): " << days * dailyRate << endl;
    outputFile << "Service charge (in ₹): " << serviceCharge << endl;
    outputFile << "Medication charge (in ₹): " << medicalCharge << endl;
    outputFile << "Total (in ₹): " << days * dailyRate + serviceCharge + medicalCharge << endl;
    outputFile.close();
}
...
{
    ofstream outputFile;
    outputFile.open("pc_17_out_patient.txt");
    if (outputFile.fail())
    {
        cout << "Error! Could not create output file. Exiting..." << endl;
        exit(0);
    ...
    outputFile << "Medication charge (in ₹): " << medicalCharge << endl;
    outputFile << "Total (in ₹): " << serviceCharge + medicalCharge << endl;
    outputFile.close();
}
